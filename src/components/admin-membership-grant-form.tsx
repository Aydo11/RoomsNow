  ...
  return (
    <details className="min-w-[18rem] text-left">
      ...
      <form action={action} className="mt-3 space-y-3 rounded-[12px] border border-line bg-surface-soft p-4">
        ...
        <Field label="Complimentary plan" name="tier" error={state.errors?.tier} required>
        ...
        <Field
          label="Expiry date"
          name="expiresOn"
          error={state.errors?.expiresOn}
          hint="Leave blank for access that continues until an admin ends it."
        >
        ...
        <div className="flex flex-wrap gap-2">
          <SubmitButton pendingLabel="Saving…" name="intent" value="GRANT">
            {currentGrant ? "Update grant" : "Grant membership"}
          </SubmitButton>
          {currentGrant && (
            <SubmitButton
              name="intent"
